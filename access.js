...
co(function* () {
   ...
   // pg.types.setTypeParser(1184, (str) => moment(str));
    ...
    if (db) {
        ...
        if (currentProcess === "PARENT") {
            ...
            console.log("DONE. Update Task Process is running as a child.");
        }
    }
    else {
...
//console.log(status);
        ...
        // 21. Remove PUSH notification token --> Logout
            ...
            data.start = new Date(Number(data.start) * 1000);
            data.end = new Date(Number(data.end) * 1000);
            ...
            let sql = data.type === "regulatory" ?
                "SELECT ROUND(AVG(CASE WHEN timestamp>=$1 AND timestamp<=$2 AND is_enabled='1' THEN history.sla end), 2) AS sla, " +
                "history.terminal_id, terminals.branch_code, terminals.branch_name, terminals.zone, terminals.subzone, terminals.location_id, " +
                "history.issue_time, history.exception " +
                "FROM history RIGHT JOIN terminals ON (history.terminal_id=terminals.terminal_id) " +
                "GROUP BY history.terminal_id, history.issue_time, history.exception, terminals.branch_code, terminals.branch_name, terminals.zone, terminals.subzone, terminals.location_id"
                :
                "SELECT ROUND(AVG(CASE WHEN timestamp>=$1 AND timestamp<=$2 AND is_enabled='1' THEN history.availability end), 2) AS availability, " +
                "ROUND(AVG(CASE WHEN timestamp>=$1 AND timestamp<=$2 AND is_enabled='1' THEN history.sla end), 2) AS sla, " +
                "history.terminal_id, terminals.branch_code, terminals.branch_name, terminals.zone, terminals.subzone, terminals.location_id, " +
                "history.issue_time, history.exception " +
                "FROM history RIGHT JOIN terminals ON (history.terminal_id=terminals.terminal_id) " +
                "GROUP BY history.terminal_id, history.issue_time, history.exception, terminals.branch_code, terminals.branch_name, terminals.zone, terminals.subzone, terminals.location_id"
        ...
        // Bulk upload atm's history
        ...
        else if (path === "/query/pos/location/list") {
            if (user.level < 1) {
                response.write('{"status": 403, "message": "Access denied. You MUST login to query terminals."}');
                return response.end();
            }
            ...
            sql = "SELECT DISTINCT lga, merchant_code FROM merchant ORDER BY lga DESC";

            let lgas = yield db.query(sql, options);
            for (let lga of lgas.rows) {

                list.push({
                    location: lga.lga,
                    merchhant_code: lga.merchant_code,
                });
            }
            response.write(JSON.stringify({status: 200, list: list}));
            return response.end();
            ...
            if (user.level < 1) {
                ...
                response.write('{"status": 422, "message": "Please provide a valid [id] of merchant to query."}');
                return response.end();
            }
            ...
            sql = "SELECT lga, merchant_code, merchant_name, merchant_code_name, merchant_category, back_acc_no, physical_addr, state_code, revenue, merchant_id FROM merchant WHERE merchant_id = $1 ORDER BY merchant_name DESC";
            options = [data.id];
            let merchants = yield db.query(sql, options);
            for (let merchant of merchants.rows) {
    ...
    getLocationTransactions: function* (location, tag) {
        if (tag === 1) {
            ...
            return {transaction_count: total_count, transaction_value: total_value, transaction_revenue: total_profit,last_updated:last_updated}
        }
        if (tag === 2) {
